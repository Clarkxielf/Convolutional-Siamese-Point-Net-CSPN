...
inspection_data = sio.loadmat('./blade3_S3_View.mat')['View'][:, :2]
...
segmented_data3 = segmented_data['segmented_3']
...
src = segmented_data2[-60:-60+32]
index = ((src[:, None, :].repeat(segmented_data3.shape[0], 1)-segmented_data3[None, ...])**2).sum(-1).argmin(axis=-1)
ref = segmented_data3[index[0]:index[0]+32]
plt.scatter(src[:, 0], src[:, 1], s=2, facecolors='none', edgecolors=color[0])
plt.scatter(ref[:, 0], ref[:, 1], s=2, facecolors='none', edgecolors=color[1])
plt.show()
all_Src.append(src[None, ...])
...
sio.savemat('./blade3_S3_inference_data_test0.mat',
            {'Src': all_Src,
             'Ref': all_Ref,
...
plt.show()
sio.savemat('./blade3_S3_preprocess.mat',
...
sio.savemat('./blade3_S3_inference_data_Segmented_idx.mat',
