...
from CSPN.src.common.colors import BLUE, ORANGE
from CSPN.src.common.misc import prepare_logger
from CSPN.src.common.torch import dict_all_to_device, CheckPointManager, TorchDebugger
from CSPN.src.common import se3
from CSPN.src.data_loader.datasets import get_train_datasets
from CSPN.src.eval import print_metrics
from CSPN.src.models.rpmnet import get_model
    ...
    with torch.no_grad():
        ...
        colors = torch.from_numpy(
            np.concatenate([np.tile(ORANGE, (*points_src.shape[:2], 1)),
                            np.tile(BLUE, (*points_ref.shape[:2], 1))], axis=1))
...
def validate(data_loader, model: torch.nn.Module, summary_writer: SummaryWriter, step: int):
    ...
    mse_losses_by_iteration = torch.stack([mean_val_losses['mse_{}'.format(k)]
                                       for k in range(_args.num_reg_iter)]).cpu().numpy()
    mae_losses_by_iteration = torch.stack([mean_val_losses['mae_{}'.format(k)]
                                       for k in range(_args.num_reg_iter)]).cpu().numpy()
    rmse_losses_by_iteration = torch.stack([torch.sqrt(mean_val_losses['mse_{}'.format(k)])
                                       for k in range(_args.num_reg_iter)]).cpu().numpy()
    r_mse_losses_by_iteration = torch.stack([mean_val_losses['r_mse_{}'.format(k)]
                                       for k in range(_args.num_reg_iter)]).cpu().numpy()
    r_mae_losses_by_iteration = torch.stack([mean_val_losses['r_mae_{}'.format(k)]
                                       for k in range(_args.num_reg_iter)]).cpu().numpy()
    r_rmse_losses_by_iteration = torch.stack([torch.sqrt(mean_val_losses['r_mse_{}'.format(k)])
                                       for k in range(_args.num_reg_iter)]).cpu().numpy()
    t_mse_losses_by_iteration = torch.stack([mean_val_losses['t_mse_{}'.format(k)]
                                       for k in range(_args.num_reg_iter)]).cpu().numpy()
    t_mae_losses_by_iteration = torch.stack([mean_val_losses['t_mae_{}'.format(k)]
                                       for k in range(_args.num_reg_iter)]).cpu().numpy()
    t_rmse_losses_by_iteration = torch.stack([torch.sqrt(mean_val_losses['t_mse_{}'.format(k)])
                                       for k in range(_args.num_reg_iter)]).cpu().numpy()
    ...
    save_summaries(summary_writer, data=data_to_rerun, predicted=pred_transforms, endpoints=endpoints,
                   losses=mean_val_losses, step=step)
