...
from CSPN.src.common.torch import to_numpy
from CSPN.src.models.pointnet_util import square_distance, angle_difference
from CSPN.src.models.feature_nets import src_DGCNN, ref_DGCNN
from CSPN.src.models.feature_nets import feat_ParameterPredictionNet, coordinate_ParameterPredictionNet
from CSPN.src.common.math_torch import se3
...
class RPMNet(nn.Module):
    ...
    def forward(self, data, num_iter: int = 1):
        ...
        for i in range(num_iter):
            ...
            coordinate_distance = match_matrix(xyz_src, xyz_ref)
            coordinate_beta, coordinate_alpha = self.coordinate_weights_net([xyz_src, xyz_ref])
            coordinate_affinity = self.compute_affinity(coordinate_beta, coordinate_distance, alpha=coordinate_alpha)
            coordinate_log_perm_matrix = sinkhorn(coordinate_affinity, n_iters=self.num_sk_iter, slack=self.add_slack)
            coordinate_perm_matrix = torch.exp(coordinate_log_perm_matrix)

            feat_src = self.src_feat_extractor(xyz_src_t)
            feat_ref = self.ref_feat_extractor(xyz_ref)
            feat_distance = match_matrix(feat_src, feat_ref)
            feat_beta, feat_alpha = self.feat_weights_net([xyz_src, xyz_ref])
            feat_affinity = self.compute_affinity(feat_beta, feat_distance, alpha=feat_alpha)
            feat_log_perm_matrix = sinkhorn(feat_affinity, n_iters=self.num_sk_iter, slack=self.add_slack)
            feat_perm_matrix = torch.exp(feat_log_perm_matrix)

            weighted_coordinate_perm_matrix = coordinate_perm_matrix.topk(k=1, dim=-1)[0]
            weighted_feat_perm_matrix = feat_perm_matrix.topk(k=1, dim=-1)[0]
            weighted_perm_matrix = torch.cat([weighted_coordinate_perm_matrix, weighted_feat_perm_matrix], -1)
            weighted_perm_matrix = torch.softmax(weighted_perm_matrix, -1)
            perm_matrix = weighted_perm_matrix[..., 0][..., None]*coordinate_perm_matrix\
                          + weighted_perm_matrix[..., 1][..., None]*feat_perm_matrix
...
class RPMNetEarlyFusion(RPMNet):
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        self.feat_weights_net = feat_ParameterPredictionNet()
        self.coordinate_weights_net = coordinate_ParameterPredictionNet()
        self.src_feat_extractor = src_DGCNN(feature_dim=args.feat_dim, num_neighbors=args.num_neighbors)
        self.ref_feat_extractor = ref_DGCNN(feature_dim=args.feat_dim, num_neighbors=args.num_neighbors)
