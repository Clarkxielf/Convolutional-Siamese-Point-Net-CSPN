...
from CSPN.src.models import rpmnet
from CSPN.src.arguments import RegistrationNet_arguments
from CSPN.src.common.misc import prepare_logger
from CSPN.src.common.torch import dict_all_to_device, CheckPointManager, to_numpy
from CSPN.src.common import se3
from CSPN.src.data_loader.datasets import get_test_datasets
...
def inference(data_loader, model: torch.nn.Module):
    ...
    with torch.no_grad():
        for val_data in tqdm(data_loader):
            ...
            for pred_transform in pred_transforms[0]:
                ...
                # plt.show()
                plt.scatter(src_transformed[:, 0], src_transformed[:, 1], s=10, color='red')
                plt.scatter(to_numpy(points_ref[i])[:, 0], to_numpy(points_ref[i])[:, 1], s=10, color='blue')
                plt.yticks([])
                plt.xticks([])
            ...
            # sio.savemat('../data/rpm_blade3_S1_inference_data_test0.mat',
            #             {'Src': all_Src,
            #              'Ref': all_Ref})

            pred_transforms_all.append(pred_transforms[0])
...
def main():
    ...
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=_args.val_batch_size, shuffle=False)

    model = get_model()
    pred_transforms = inference(test_loader, model)  # Feedforward transforms

    all_Segmented_idx = sio.loadmat('../data/blade1_S3_inference_data_Segmented_idx.mat')['all_Segmented_idx'][0]
    i = 2
    j = 0
    for inspection_data_path in glob.glob(os.path.join('../data', 'blade1_S*_preprocess.mat')):
        ...
        plt.scatter(pred_Inspection_segmented_data_i[..., 0], pred_Inspection_segmented_data_i[..., 1], s=10, color='red')
        ...
        pred_Inspection_segmented_data_i = np.concatenate([pred_Inspection_segmented_data_i,
                                                           np.ones((pred_Inspection_segmented_data_i.shape[0], 1))*102.03], axis=-1)


        # sio.savemat('../data/rpm_blade3_S1_preprocess.mat',
        #             {'inspection_data': pred_Inspection_segmented_data_i[..., :2]})
